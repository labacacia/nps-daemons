...
- **处理** NCP 握手（含 [NPS-RFC-0001](https://gitee.com/labacacia/NPS-Release/blob/main/spec/rfcs/NPS-RFC-0001-ncp-connection-preamble.cn.md) 连接前导）、帧编解码、AnnounceFrame 发布、root Ed25519 密钥对管理、为本机 agent 签发 sub-NID、本机 session 注册表、按 NID 维护 inbox 队列（`ephemeral` 激活模式投递）。
...
- **参考合规**：目标是 `NPS-Node Profile L1`（见 [`spec/services/NPS-Node-Profile.cn.md`](https://gitee.com/labacacia/NPS-Release/blob/main/spec/services/NPS-Node-Profile.cn.md) 与 [`spec/services/conformance/NPS-Node-L1.cn.md`](https://gitee.com/labacacia/NPS-Release/blob/main/spec/services/conformance/NPS-Node-L1.cn.md)）。
